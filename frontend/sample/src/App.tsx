...
const AppContent: React.FC = () => {
  const { currentUser, users, setCurrentUserById, logout, isLoading } = useAuth();
              ...
              <div className="relative">
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-72 origin-top-right rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none divide-y divide-slate-100/60 dark:divide-slate-800 transition-all duration-200">
                    ...
                    <div className="py-1 bg-red-50/10 dark:bg-red-950/5">
                      <button
                        onClick={() => {
                          logout();
                          setDropdownOpen(false);
                        }}
                        className="w-full text-left px-4 py-2.5 text-xs text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors font-bold flex items-center gap-2"
                      >
                        <ShieldAlert size={14} className="text-red-400" /> Log Out
                      </button>
                    </div>
                  </div>
                )}
